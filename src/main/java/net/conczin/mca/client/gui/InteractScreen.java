...
public class InteractScreen extends AbstractDynamicScreen {
    ...
    public boolean mouseScrolled(double x, double y, double dx, double dy) {
        // 1.21.11: Inventory.selected requires access widener (mca.accesswidener)
        var inventory = player.getInventory();
        if (dy < 0) {
            inventory.selected = inventory.selected == 8 ? 0 : inventory.selected + 1;
        } else if (dy > 0) {
            inventory.selected = inventory.selected == 0 ? 8 : inventory.selected - 1;
        }
    ...
    public boolean mouseClicked(double posX, double posY, int button) {
        ...
        if (button == 0 && dialogAnswerHover != null && dialogQuestionText != null) {
            // Note: double click would require using a different event (7.4.0)
