...
public class GrimReaperMeleeGoal extends Goal {
    ...
    private void curse() {
        ...
        if (entityToAttack instanceof Player player) {
            ...
            // Also, randomly swap their selected item with something else in the hotbar and
            // apply blindness.
    ...
    public void tick() {
        ...
        if (blockDuration > 0) {
            ...
            if (reaper.distanceToSqr(entityToAttack) <= 1.0D) {
                ...
                entityToAttack.hurt(reaper.level().damageSources().mobAttack(reaper),
                        (float) reaper.getAttributeValue(Attributes.ATTACK_DAMAGE));
