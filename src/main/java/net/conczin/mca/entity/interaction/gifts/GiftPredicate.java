...
public class GiftPredicate {
        ...
        static {
                ...
                register("age_group",
                                (json, name) -> AgeState.valueOf(
                                                GsonHelper.convertToString(json, name).toUpperCase(Locale.ENGLISH)),
                                group -> (villager, stack, player) -> villager.getAgeState() == group ? 1.0f : 0.0f);
                register("gender",
                                (json, name) -> Gender.valueOf(
                                                GsonHelper.convertToString(json, name).toUpperCase(Locale.ENGLISH)),
                                gender -> (villager, stack,
                                                player) -> villager.getGenetics().getGender() == gender ? 1.0f : 0.0f);
                ...
                register("has_village", GsonHelper::convertToBoolean, hasVillage -> (villager, stack,
                                player) -> villager.getResidency().getHomeVillage().isPresent() == hasVillage ? 1.0f
                                                : 0.0f);
                register("min_infection_progress", GsonHelper::convertToFloat,
                                progress -> (villager, stack,
                                                player) -> villager.getInfectionProgress() > progress ? 1.0f : 0.0f);
                register("mood", (json, name) -> GsonHelper.convertToString(json, name).toLowerCase(Locale.ENGLISH),
                                mood -> (villager, stack,
                                                player) -> villager.getVillagerBrain().getMood().getName().equals(mood)
                                                                ? 1.0f
                                                                : 0.0f);
                register("personality",
                                (json, name) -> Personality.valueOf(
                                                GsonHelper.convertToString(json, name).toUpperCase(Locale.ENGLISH)),
                                personality -> (villager, stack,
                                                player) -> villager.getVillagerBrain().getPersonality() == personality
                                                                ? 1.0f
                                                                : 0.0f);
                register("is_pregnant", GsonHelper::convertToBoolean, pregnant -> (villager, stack,
                                player) -> villager.getRelationships().getPregnancy().isPregnant() == pregnant ? 1.0f
                                                : 0.0f);
                register("min_pregnancy_progress", GsonHelper::convertToInt, progress -> (villager, stack,
                                player) -> villager.getRelationships().getPregnancy().getBabyAge() > progress ? 1.0f
                                                : 0.0f);
                register("pregnancy_child_gender",
                                (json, name) -> Gender.valueOf(
                                                GsonHelper.convertToString(json, name).toUpperCase(Locale.ENGLISH)),
                                gender -> (villager, stack,
                                                player) -> villager.getRelationships().getPregnancy()
                                                                .getGender() == gender ? 1.0f : 0.0f);
                register("current_chore",
                                (json, name) -> Chore.valueOf(
                                                GsonHelper.convertToString(json, name).toUpperCase(Locale.ENGLISH)),
                                chore -> (villager, stack,
                                                player) -> villager.getVillagerBrain().getCurrentJob() == chore ? 1.0f
                                                                : 0.0f);
                ...
                register("tag", (json, name) -> {
                        ...
                        // constructor
                        // In 1.21.11, Ingredient.of(TagKey) not available, test directly
                        return tag; // Return the tag itself, we'll test differently
                }, (TagKey<Item> tag) -> (villager, stack, player) -> stack.is(tag) ? 1.0f : 0.0f);
                register("trait",
                                (json, name) -> Traits.Trait
                                                .valueOf(GsonHelper.convertToString(json, name)
                                                                .toUpperCase(Locale.ENGLISH)),
                                trait -> (villager, stack, player) -> villager.getTraits().hasTrait(trait) ? 1.0f
                                                : 0.0f);
                ...
                register("emeralds", GsonHelper::convertToInt, amount -> (villager, stack,
                                player) -> (player != null && player.getInventory().countItem(Items.EMERALD) >= amount)
                                                ? 1.0f
                                                : 0.0f);
                register("village_has_building", GsonHelper::convertToString, name -> (villager, stack,
                                player) -> villager.getResidency().getHomeVillage().filter(v -> v.hasBuilding(name))
                                                .isPresent() ? 1.0f
                                                                : 0.0f);
                register("rank", GsonHelper::convertToString,
                                name -> (villager, stack,
                                                player) -> villager.getResidency().getHomeVillage()
                                                                .filter(v -> Tasks.getRank(v, player) == Rank
                                                                                .fromName(name))
                                                                .isPresent() ? 1.0f
                                                                                : 0.0f);
        ...
        public static <T> void register(String name, BiFunction<JsonElement, String, T> jsonParser,
                        Factory<T> predicate) {
        ...
        public interface Condition {
                ...
                default Condition and(Condition b) {
                        ...
                        return (villager, stack, player) -> a.test(villager, stack, player)
                                        * b.test(villager, stack, player);
                }
        }
}
