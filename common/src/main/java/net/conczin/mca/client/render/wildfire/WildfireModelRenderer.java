...
import net.conczin.mca.client.render.wildfire.uv.UVDirection;
...
public final class WildfireModelRenderer {
    ...
    public static class ModelBox {
        ...
        protected ModelBox(int tW, int tH, float x, float y, float z, int dx, int dy, int dz, float delta, int quads,
        ...
        protected void initQuads(int tW, int tH, int dx, int dy, int dz, int quads,
            ...
            for (Map.Entry<UVDirection, UVQuad> entry : dynamicUvLayouts.getAllSides().entrySet()) {
                ...
                if (quad != null) { // Check for null as fillMissing might put nulls
    ...
    public static class TexturedQuad {
        ...
        public TexturedQuad(float u1, float v1, float u2, float v2, float texWidth, float texHeight,
                UVDirection directionIn, PositionTextureVertex... positionsIn) {
            Preconditions.checkArgument(positionsIn.length == 4, "Incorrect number of vertices; expected 4, got %s",
                    positionsIn.length);
